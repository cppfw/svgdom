...
const tst::set set("finders", [](auto& suite) {
	suite.add("finder_by_id", []() {
		...
		tst::check(
			finder_by_id.size() == 2,
			[&](auto& o) {
				o << "finder_by_id size = " << finder_by_id.size();
			},
			SL
		);
		...
		for (const auto& id : {"id1", "id2"}) {
			...
			tst::check(
				element_by_id != nullptr,
				[&](auto& o) {
					o << "element-by-id not found, searched id = " << id;
				},
				SL
			);
			tst::check(
				element_by_id->id == id,
				SL
			); // NOLINT(clang-analyzer-core.NonNullParamChecker): element_by_id is not nullptr as per previous check
		}
	...
	suite.add("finder_by_class", []() {
		...
		tst::check(
			finder_by_class_name.size() == 2,
			[&](auto& o) {
				o << "finder_by_class_name size = " << finder_by_class_name.size();
			},
			SL
		);
		...
		for (const auto& cls : {"class1", "class2"}) {
			...
			tst::check(
				!elements_by_class_name.empty(),
				[&](auto& o) {
					o << "elements-by-class not found, searched class = " << cls;
				},
				SL
			);
	...
	suite.add("finder_by_tag", []() {
		...
		tst::check(
			finder_by_tag_name.size() == 3,
			[&](auto& o) {
				o << "finder_by_tag_name size = " << finder_by_tag_name.size();
			},
			SL
		);
		...
		for (const auto& t : {"circle", "svg", "rect"}) {
			...
			tst::check(
				!elements_by_tag_name.empty(),
				[&](auto& o) {
					o << "elements-by-tag not found";
				},
				SL
			);
