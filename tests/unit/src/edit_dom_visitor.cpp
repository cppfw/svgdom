...
const tst::set set("edit_dom_visitor", [](auto& suite) {
	suite.add("basic_test", []() {
		...
		tst::check_eq(
			dynamic_cast<svgdom::g_element*>((++ ++dom->children.begin())->get())->children.size(),
			size_t(1),
			SL
		);
		tst::check(
			dynamic_cast<svgdom::line_element*>(
				dynamic_cast<svgdom::g_element*>((++ ++dom->children.begin())->get())->children.front().get()
			),
			SL
		);
		...
		tst::check_eq(
			...
			size_t(2),
			[&](auto& o) {
				o << "dom->children.size() = " << dom->children.size();
			},
			SL
		);
		tst::check(dynamic_cast<svgdom::path_element*>(dom->children.begin()->get()), SL);
