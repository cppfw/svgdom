...
)
{
	...
	tst::check(
		...
		[&](auto& o) {
			o << "parsed value for '" << styleable::property_to_string(p)
			  << "' does not hold the expected enumeration type" << std::endl;
...
const tst::set set("style_properties", [](tst::suite& suite) {
	suite.add("keyword_properties_roundtrip", []() {
		// color-rendering
		check_keyword_roundtrip<color_rendering>(
			style_property::color_rendering,
			"auto",
			&parse_color_rendering,
			&color_rendering_to_string
		);
		check_keyword_roundtrip<color_rendering>(
			style_property::color_rendering,
			"optimizeSpeed",
		...
		// font-stretch
		check_keyword_roundtrip<font_stretch>(
			style_property::font_stretch,
			"normal",
			&parse_font_stretch,
			&font_stretch_to_string
		);
		check_keyword_roundtrip<font_stretch>(
			style_property::font_stretch,
			"wider",
		...
		// font-style
		check_keyword_roundtrip<font_style>(
			style_property::font_style,
			"normal",
			&parse_font_style,
			&font_style_to_string
		);
		check_keyword_roundtrip<font_style>(
			style_property::font_style,
			"italic",
		...
		// font-variant
		check_keyword_roundtrip<font_variant>(
			style_property::font_variant,
			"normal",
			&parse_font_variant,
			&font_variant_to_string
		);
		check_keyword_roundtrip<font_variant>(
			style_property::font_variant,
			"small-caps",
		...
		// image-rendering
		check_keyword_roundtrip<image_rendering>(
			style_property::image_rendering,
			"auto",
			&parse_image_rendering,
			&image_rendering_to_string
		);
		check_keyword_roundtrip<image_rendering>(
			style_property::image_rendering,
			"optimizeSpeed",
		...
		// shape-rendering
		check_keyword_roundtrip<shape_rendering>(
			style_property::shape_rendering,
			"auto",
			&parse_shape_rendering,
			&shape_rendering_to_string
		);
		check_keyword_roundtrip<shape_rendering>(
			style_property::shape_rendering,
			"optimizeSpeed",
		...
		// text-anchor
		check_keyword_roundtrip<text_anchor>(
			style_property::text_anchor,
			"start",
			&parse_text_anchor,
			&text_anchor_to_string
		);
		check_keyword_roundtrip<text_anchor>(
			style_property::text_anchor,
			"middle",
		...
		// text-rendering
		check_keyword_roundtrip<text_rendering>(
			style_property::text_rendering,
			"auto",
			&parse_text_rendering,
			&text_rendering_to_string
		);
		check_keyword_roundtrip<text_rendering>(
			style_property::text_rendering,
			"optimizeSpeed",
	...
	suite.add("font_weight_roundtrip", []() {
	...
	suite.add("inherit_value_roundtrip", []() {
	...
	suite.add("parse_full_style_attribute", []() {
		...
		tst::check_eq(
			styleable::style_value_to_string(
				style_property::color_rendering,
				styles.at(style_property::color_rendering)
			),
			std::string("optimizeQuality"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(style_property::font_stretch, styles.at(style_property::font_stretch)),
			std::string("expanded"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(style_property::font_style, styles.at(style_property::font_style)),
			std::string("oblique"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(style_property::font_variant, styles.at(style_property::font_variant)),
			std::string("small-caps"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(style_property::font_weight, styles.at(style_property::font_weight)),
			std::string("bold"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(
				style_property::image_rendering,
				styles.at(style_property::image_rendering)
			),
			std::string("optimizeQuality"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(style_property::overflow, styles.at(style_property::overflow)),
			std::string("visible"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(
				style_property::shape_rendering,
				styles.at(style_property::shape_rendering)
			),
			std::string("geometricPrecision"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(style_property::text_anchor, styles.at(style_property::text_anchor)),
			std::string("end"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(style_property::text_rendering, styles.at(style_property::text_rendering)),
			std::string("optimizeLegibility"),
			SL
		);
	});

	suite.add("svg_roundtrip_via_to_string", []() {
		std::string_view svg =
			R"xxx(
		...
		tst::check_eq(
			std::string(overflow_to_string(*st->get_style_property(style_property::overflow))),
			std::string("hidden"),
			SL
		);
		tst::check_eq(
			std::string(text_anchor_to_string(*st->get_style_property(style_property::text_anchor))),
			std::string("middle"),
			SL
		);
		tst::check_eq(
			std::string(font_style_to_string(*st->get_style_property(style_property::font_style))),
			std::string("italic"),
			SL
		);
		tst::check_eq(
			std::string(shape_rendering_to_string(*st->get_style_property(style_property::shape_rendering))),
			std::string("crispEdges"),
			SL
		);
		tst::check_eq(
			styleable::style_value_to_string(
				style_property::font_weight,
				*st->get_style_property(style_property::font_weight)
			),
			std::string("bold"),
			SL
		);
	...
	suite.add("compositing_blending_properties", [] {
		...
		check_keyword_roundtrip<mix_blend_mode>(
			style_property::mix_blend_mode,
			"normal",
			&parse_mix_blend_mode,
			&mix_blend_mode_to_string
		);
		check_keyword_roundtrip<mix_blend_mode>(
			style_property::mix_blend_mode,
			"multiply",
		...
		// the 'inherit' keyword is still handled generically for these properties
		{
			...
			tst::check_eq(
				styleable::style_value_to_string(style_property::mix_blend_mode, v),
				std::string("inherit"),
				SL
			);
		}
